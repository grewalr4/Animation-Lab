...
void Picture::Save(const wxString& filename)
{
    ...
    //
    root->AddAttribute(L"laboratoryNumber", to_string(GetLaboratoryDrawable()->GetLabNum()));
...
void Picture::Load(const wxString& filename)
{
    ...
    //
    int laboratoryNumber = stoi(root->GetAttribute(L"laboratoryNumber", L"1").ToStdWstring());
    GetLaboratoryDrawable()->SetLabNum(laboratoryNumber);
