...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnSelectLab, this, XRCID("SelectLab"));
}
...
/**
 * Handle the Laboratory>Select Lab menu option
 * @param event The menu event
 */
void ViewEdit::OnSelectLab(wxCommandEvent& event)
{
    GetPicture()->GetLaboratoryDrawable()->SelectLab(mParent);
}
